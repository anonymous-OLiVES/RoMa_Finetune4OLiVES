...
def get_model(pretrained_backbone=True, resolution = "medium", contrastive_layers = ["4", "8"], **kwargs):
    import warnings
    ...
    dim_dict={"1":64, "2":128, "4":256, "8":512}
    projector_list = nn.ModuleDict()
    predictor_list = None
    for layer_name in contrastive_layers:
        fea_dim = dim_dict.get(layer_name)
        projector = Projector(fea_dim, amp = True)
        projector_list[layer_name] = projector
    name = kwargs.get("experiment_name")
    matcher = RegressionMatcher(encoder, decoder, h=h, w=w, attenuate_cert=kwargs["attenuate_cert"], name=name,
                                projector_list=projector_list,
                                predictor_list=predictor_list)
    if kwargs["load_pretrain"]:
        weights = torch.hub.load_state_dict_from_url(
                weight_urls["romatch"]["outdoor"], map_location="cpu")
        missing, unexpected = matcher.load_state_dict(weights, strict=False)
...
def train(args):
    ...
    checkpoint_dir = "workspace/checkpoints/" + experiment_name + "/" #+time.strftime("%Y%m%d-%H%M%S")+"/"
    h,w = resolutions[resolution]
    # get layers
    contrastive_layers = [x.strip() for x in args.contrastive_layers.split(",")]
    print(f"Using contrastive layers: {contrastive_layers}")
    model = get_model(pretrained_backbone=True, resolution=resolution, attenuate_cert = False, load_pretrain = args.load_pretrain, contrastive_layers=contrastive_layers).to(device_id)
    # Num steps
    global_step = 0
    ...
    # Data
    mega = MegadepthBuilder(data_root=args.data_root, loftr_ignore=True, imc21_ignore = True)
    ...
    megadepth_train1 = mega.build_scenes(
        ...
        ht=h,wt=w,is_degrade=args.is_degrade,degrade_params_p=args.degrade_params_p
    )
    ...
    if not args.load_pretrain:
        parameters = [
            ...
            {"params": model.projector_list.parameters(), "lr": romatch.STEP_SIZE * 1e-4 / 8},
            # {"params": model.predictor_list.parameters(), "lr": romatch.STEP_SIZE * 1e-4 / 8},
        ]
    else:
        parameters = [
            {"params": model.encoder.parameters(), "lr": romatch.STEP_SIZE * 5e-7 / 8},
            {"params": model.decoder.parameters(), "lr": romatch.STEP_SIZE * 1e-5 / 8},
            {"params": model.projector_list.parameters(), "lr": romatch.STEP_SIZE * 1e-5 / 8},
            # {"params": model.predictor_list.parameters(), "lr": romatch.STEP_SIZE * 5e-6 / 8},
        ]

    optimizer = torch.optim.AdamW(parameters, weight_decay=0.01)
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(
        optimizer, milestones=[(9*N/romatch.STEP_SIZE)//10])
    megadense_benchmark = MegadepthDenseBenchmark(args.data_root, num_samples = 1000, h=h,w=w,is_degrade=args.is_degrade,degrade_params_p=args.degrade_params_p)
    ...
    for n in range(romatch.GLOBAL_STEP, N, k * romatch.STEP_SIZE):
        ...
        mega_dataloader = iter(
            torch.utils.data.DataLoader(
                ...
                num_workers = 1 if args.debug_mode else 16,
            )
        )
        train_k_steps(
        ...
        print(f"Step {romatch.GLOBAL_STEP}: {megadense_benchmark.benchmark(model)}")
