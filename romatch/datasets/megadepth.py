...
import math
from romatch.utils import degradation
...
class MegadepthScene:
    def __init__(
        ...
        if self.is_degrade:
            self.degrade_params = degradation.parse_params(degrade_params_p)
        else:
            self.degrade_params = None

    def load_im(self, im_path):
        im = Image.open(im_path)
        return im
    
    def horizontal_flip(self, im_list, depth_A, depth_B,  K_A, K_B):
        for idx in range(len(im_list)):
            im_list[idx] =  im_list[idx].flip(-1)
        depth_A, depth_B = depth_A.flip(-1), depth_B.flip(-1) 
        flip_mat = torch.tensor([[-1, 0, self.wt],[0,1,0],[0,0,1.]]).to(K_A.device)
        K_A = flip_mat@K_A  
        K_B = flip_mat@K_B  
        
        return im_list, depth_A, depth_B, K_A, K_B
    ...
    def __getitem__(self, pair_idx):
        ...
        """ degrade for im_A """
        if self.is_degrade:
            im_A1, mode1 = degradation.generate_noise_for_train(im_A,self.degrade_params,device=im_A.device, use_normal=False)
            im_A2, mode2 = degradation.generate_noise_for_train(im_A,self.degrade_params,device=im_A.device, use_normal=True)
            if romatch.DEBUG_MODE:
                im_A_vis = tensor_to_pil(im_A, unnormalize=True)
                im_A1_vis = tensor_to_pil(im_A1, unnormalize=True)
                im_A2_vis = tensor_to_pil(im_A2, unnormalize=True)
                # im_A_vis.save(f"{self.scene_name}_{idx1}_degraded.jpg")

        depth_A, depth_B = self.depth_transform_ops(
            (depth_A[None, None], depth_B[None, None])
        )
        
        [im_A1, im_A2, im_B, depth_A, depth_B], t = self.rand_shake(im_A1, im_A2, im_B, depth_A, depth_B)
        K1[:2, 2] += t
        K2[:2, 2] += t
        
        im_A1, im_A2, im_B = im_A1[None], im_A2[None], im_B[None]

        if self.use_horizontal_flip_aug:
            if np.random.rand() > 0.5:
                [im_A1, im_A2, im_B], depth_A, depth_B, K1, K2 = self.horizontal_flip([im_A1, im_A2, im_B], depth_A, depth_B, K1, K2)
        
        if romatch.DEBUG_MODE:
            tensor_to_pil(im_A1[0], unnormalize=True).save(
                            f"vis/1_{os.path.basename(im_A_ref)}")
            tensor_to_pil(im_A2[0], unnormalize=True).save(
                            f"vis/2_{os.path.basename(im_A_ref)}")
