...
class MegaDepthPoseEstimationBenchmark:
    ...
    def benchmark(self, model, model_name = None, mode=""):
        with torch.no_grad():
            data_root = self.data_root
            tot_e_t, tot_e_R, tot_e_pose = [], [], []
            thresholds = [5, 10, 20]
            print(f"Evaluating {model_name} on MegaDepth Pose Estimation Benchmark with mode {mode}...")
            for scene_ind in range(len(self.scenes)):
                ...
                #         if im_path is not None and "Undistorted_SfM/" in im_path:
                #             im_path = im_path.replace("Undistorted_SfM/", f"Undistorted_SfM_{mode}/")
                #             assert os.path.exists(f"{data_root}/{im_path}"), f"{data_root}/{im_path} does not exist"
                #             print(im_path)
                #             im_paths[idx] = im_path
                ...
                for pairind in (pbar := tqdm(pair_inds, desc = "Current AUC: ?")):
                    ...
                    if not mode == "":
                        im_A_path = im_A_path.replace("Undistorted_SfM/", f"Undistorted_SfM_{mode}/")
                        assert os.path.exists(im_A_path), f"{im_A_path} does not exist"
