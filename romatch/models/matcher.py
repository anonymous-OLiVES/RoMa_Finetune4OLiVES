...
class RegressionMatcher(nn.Module):
    ...
    def extract_backbone_features(self, batch, batched=True, upsample=False):
        ...
        if batched:
            if self.training:
                x_a = batch["im_A2"]
                X = X = torch.cat((x_q, x_s, x_a), dim=0)
            else:
    ...
    def cmp_aug_feat(self, f_q_pyramid, f_a_pyramid):
        ret = {}
        for scale in self.projector_list:
            projector = self.projector_list[scale]
            # predictor = self.predictor_list[scale]

            f_q = f_q_pyramid[int(scale)]
            f_a = f_a_pyramid[int(scale)]
    ...
    def forward(self, batch, batched=True, upsample=False, scale_factor=1):
        ...
        if batched:
            if self.training:
                f_q_pyramid = {
                    scale: f_scale.chunk(3)[0] for scale, f_scale in feature_pyramid.items()
                }
                f_s_pyramid = {
                    scale: f_scale.chunk(3)[1] for scale, f_scale in feature_pyramid.items()
                }
                f_a_pyramid = {
                    scale: f_scale.chunk(3)[2] for scale, f_scale in feature_pyramid.items()
                }
            else:
                f_q_pyramid = {
        ...
        if self.training:
            cmp_feat_dict = self.cmp_aug_feat(f_q_pyramid, f_a_pyramid)
            for k,v in cmp_feat_dict.items():
                corresps[k] = v
        return corresps
